# IDEA 安装
<!-- more -->

### 1. [下载IDEA](https://www.jetbrains.com/idea/download/#section=windows)(本教程之适用于2022.2版本)
### 2. 下载完之后 双击安装
### 3. 勾选 `Intelli IDEA` 之后点击下一步
![图片](./software-installation/imags/IDEA/1.png)
### 4. 点击 Install 
![图片](./software-installation/imags/IDEA/2.png)
### 5. 勾选 `Run Intelli IDEA` 之后点击完成
 ![图片](./software-installation/imags/IDEA/3.png)
### 6. 不要登录！点击 Exit 退出破解
![图片](./software-installation/imags/IDEA/4.png)
### 7. [下载 解压破解包（提取码：8tz4）](https://pan.baidu.com/s/12sWQtN_yvqwJIVMHkiYoXw?pwd=8tz4)（注意：将ja-netfilter文件夹移动到某个位置就不要动了！）
![图片](./software-installation/imags/IDEA/5.png)
### 8. 找到安装 IDEA 的路径，用记事本打开 `idea64.exe.vmoptions` 配置文件，
![图片](./software-installation/imags/IDEA/6.png)
### 9.   在 `idea64.exe.vmoptions` 文件结尾添加如下配置：
```perl
# 补丁的绝对路径（可根据你实际的位置进行修改）,注意路径一定要填写正确，且不能包含中文，否则会导致 IDEA 无法启动
-javaagent:D:/ja-netfilter/ja-netfilter.jar

# 最新 IDEA 版本需要添加下面两行，否则会报 key valid
--add-opens=java.base/jdk.internal.org.objectweb.asm=ALL-UNNAMED
--add-opens=java.base/jdk.internal.org.objectweb.asm.tree=ALL-UNNAMED
```
![图片](./software-installation/imags/IDEA/7.png)
### 10. 全选复制下面激活码 看图片步骤激活
```
ZCB571FZHV---
```
![图片](./software-installation/imags/IDEA/8.png)
### 11. 这样就破解成功了！
![图片](./software-installation/imags/IDEA/9.png)